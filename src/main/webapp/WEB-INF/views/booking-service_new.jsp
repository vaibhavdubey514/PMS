...
<div class="container">
    ...
    <input type="text" name="receiverPin" id="receiverPin" placeholder="Enter Receiver Pin" pattern="\d{6}" oninput="this.value=this.value.slice(0,6)" required />
    <small class="invalid-feedback">Exactly 6 digits</small>
		...
		<div class="row">
			...
			<p class="col">
				...
				<input type="tel" pattern="[0-9]{10}" minlength="10" maxlength="10" name="receiverMobileNumber" id="receiverMobileNumber" placeholder="Enter Receiver Mobile Number" oninput="this.value=this.value.slice(0,10)" required />
				<small class="invalid-feedback">Exactly 10 digits</small>
			</p>
			<p class="col">
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    function updateDateConstraints() {
        const now = new Date();
        const isoNow = now.toISOString().slice(0, 16); // Get current date and time in ISO format
        pickup.min = isoNow; // Set min for pickup
        dropoff.min = isoNow; // Set min for dropoff initially

        pickup.addEventListener("change", () => {
            const pickupTime = pickup.value;
            if (pickupTime) {
                dropoff.min = pickupTime; // Update dropoff min to pickup time
